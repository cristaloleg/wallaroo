...
// Python 3 specific
use @py_bytes_or_unicode_size[USize](str: Pointer[U8] tag) if "python3"
use @py_bytes_or_unicode_as_char[Pointer[U8]](str: Pointer[U8] tag)
  if "python3"
use @PyBytes_Size[USize](str: Pointer[U8] box) if "python3"
use @PyUnicode_GetLength[USize](str: Pointer[U8] box) if "python3"
use @PyUnicode_AsUTF8[Pointer[U8]](str: Pointer[U8] box) if "python3"
use @PyUnicode_FromStringAndSize[Pointer[U8]](str: Pointer[U8] tag,
  size: USize) if "python3"
use @PyLong_AsLong[I64](i: Pointer[U8] box) if "python3"

// Python 2 specific
use @Py_SetProgramName[None](str: Pointer[U8] tag) if "python2"
use @PySys_SetArgv[None](argc: U32, argv: Pointer[U8] tag) if "python2"
use @PyString_Size[USize](str: Pointer[U8] box) if "python2"
use @PyString_AsString[Pointer[U8]](str: Pointer[U8] box) if "python2"
use @PyString_FromStringAndSize[Pointer[U8]](str: Pointer[U8] tag,
  size: USize) if "python2"
use @PyInt_AsLong[I64](i: Pointer[U8] box) if "python2"
...
class PyFramedSourceHandler is FramedSourceHandler[(PyData val | None)]
  ...
  fun payload_length(data: Array[U8] iso): USize =>
    Machida.framed_source_decoder_payload_length(_source_decoder,
...
class PyTCPEncoder is TCPSinkEncoder[PyData val]
  ...
  fun apply(data: PyData val, wb: Writer): Array[ByteSeq] val =>
    ...
    if not Machida.is_py_none(byte_buffer) then
      let byte_string =
        ifdef "python2" then
          @PyString_AsString(byte_buffer)
        elseif "python3" then
          @py_bytes_or_unicode_as_char(byte_buffer)
        else
          Fail(); recover Pointer[U8] end
        end
...
primitive Machida
  ...
  =>
    ...
    for i in Range(0, size) do
      ...
      ifdef "python2" then
        arr.push(recover String.copy_cstring(@PyString_AsString(ps)) end)
      elseif "python3" then
        arr.push(Machida.py_bytes_or_unicode_to_pony_string(ps))
      end
    end
  ...
  =>
    ...
    for (i, v) in args.pairs() do
      ifdef "python2" then
        @PyList_SetItem(l, i, @PyString_FromStringAndSize(v.cstring(),
          v.size()))
      elseif "python3" then
        @PyList_SetItem(l, i, @PyUnicode_FromStringAndSize(v.cstring(),
          v.size()))
      end
    end
    l
...
primitive _SourceConfig
  ...
  =>
    let name = recover val
      ifdef "python2" then
        String.copy_cstring(@PyString_AsString(@PyTuple_GetItem(
          source_config_tuple, 0)))
      elseif "python3" then
        Machida.py_bytes_or_unicode_to_pony_string(@PyTuple_GetItem(
          source_config_tuple, 0))
      else
        Fail(); ""
      end
  ...
  fun _kafka_config_options(source_config_tuple: Pointer[U8] val): KafkaConfigOptions iso^ =>
    ...
    let brokers = recover val
      ...
      for i in Range(0, num_brokers) do
        ...
        let host =
          ifdef "python2" then
            recover val String.copy_cstring(@PyString_AsString(
              @PyTuple_GetItem(broker, 0))) end
          elseif "python3" then
            recover val Machida.py_bytes_or_unicode_to_pony_string(
              @PyTuple_GetItem(broker, 0)) end
          else
            Fail(); ""
          end
        let port = try
          ifdef "python2" then
            String.copy_cstring(@PyString_AsString(@PyTuple_GetItem(broker,
              1))).i32()?
          elseif "python3" then
            Machida.py_bytes_or_unicode_to_pony_string(@PyTuple_GetItem(broker,
              1)).i32()?
          else
            Fail(); 0
          end
...
primitive _SinkConfig
  ...
  fun _kafka_config_options(source_config_tuple: Pointer[U8] val): KafkaConfigOptions iso^ =>
    ...
    let max_produce_buffer_ms =
      ifdef "python2" then
        @PyInt_AsLong(@PyTuple_GetItem(source_config_tuple, 4)).u64()
      elseif "python3" then
        @PyLong_AsLong(@PyTuple_GetItem(source_config_tuple, 4)).u64()
      else
        Fail(); 0
      end
    ...
    let max_message_size =
      ifdef "python2" then
        @PyInt_AsLong(@PyTuple_GetItem(source_config_tuple, 5)).i32()
      elseif "python3" then
        @PyLong_AsLong(@PyTuple_GetItem(source_config_tuple, 5)).i32()
      else
        Fail(); 0
      end

    KafkaConfigOptions("Wallaroo Kafka Sink", KafkaProduceOnly, topic, brokers,
      log_level, max_produce_buffer_ms, max_message_size)
...
class val PyPipelineTree
  ...
  new val create(p_graph: Pointer[U8] val, env': Env )=>
    ...
    root_idx =
      ifdef "python2" then
        @PyInt_AsLong(@PyTuple_GetItem(p_graph, 1)).usize()
      elseif "python3" then
        @PyLong_AsLong(@PyTuple_GetItem(p_graph, 1)).usize()
      else
        Fail(); 0
      end
  ...
  =>
    ...
    let command =
      ifdef "python2" then
        String.copy_cstring(command_p)
      elseif "python3" then
        Machida.py_bytes_or_unicode_to_pony_string(command_p)
      else
        Fail(); ""
      end
    match command
